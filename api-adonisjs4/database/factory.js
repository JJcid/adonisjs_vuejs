...
Factory.blueprint('App/Models/Cinema', async (faker) => {
  ...
  return {
    ...
    cinema_seat_capacity: faker.integer({
      min: 300,
      max: 1000
    }),
    cinema_details: faker.sentence({
      words: 30
    }),
  }
});
...
Factory.blueprint('App/Models/Movie', async (faker) => {
  const movie_name = faker.sentence({
    words: 3
  });
  return {
...
Factory.blueprint('App/Models/Seat', async (faker, i, data) => {
  return {
    ...
    seat_row: faker.integer({
      min: 1,
      max: 10
    }),
    seat_number: faker.integer({
      min: 1,
      max: 100
    }),
    seat_state: "BOOKED"
